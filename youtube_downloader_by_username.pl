...
my $YOUTUBE_FEEDS_CACHE = "/var/lib/mldonkey/youtube_feeds.cache";
my $GET_YOUTUBE_VIDEO_URL = "/var/lib/mldonkey/get_youtube_video_url.php";
my $TARGET_DIR = "/media/daten/mldonkey/incoming/files";
my $DEBUG = 1;
...
#	print "Date: ", $feed->pubDate(), "\n";
	foreach my $item ( @items ) {
		...
		if (exists $cache{$url}) {
			...
			# my ($video_url, $type) = split(/\ /, qx($GET_YOUTUBE_VIDEO_URL $url));
			#chomp($type);
